...
(define (compile-doc-data dat)
  ...
  (define type-info (type-label/basic type))
  (println type-info)
...
(define (compile-doc-func ent)
  ...
  (define output (func-out ((doc-prop-value type-prop))))
  (define output-string (type-label output))
  (define desc-prop? (mk-prop? 'desc))
  (define desc-prop (extract desc-prop? props))
  (define purp (doc-prop-value desc-prop))
  (string-append "@defproc[(" name " " args-string ") " output-string "? ]{\n" purp "}\n"))
