...
(define-data OpChain
  ...
  [Examples:
   ...
   1 + 2 * 3 <=
   (cons 1 (cons '+ (cons 2 (cons '* (cons 3 '())))))
   -3.5 / 1/2 <= (cons -3.5 (cons '/ (cons 1/2 '())))])
...
(define-data Tree
  [Signature: {X} (tree X [Listof [Tree X]])]
  [Purpose:
   A tree.
...
(define-docs (annotate-depth t0)
  [Signature: {X} [Tree X] -> [Tree (list X Nat)]]
  [Purpose:
